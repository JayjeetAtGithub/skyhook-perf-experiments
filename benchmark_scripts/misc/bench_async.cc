...
std::shared_ptr<ds::Dataset> GetDatasetFromDirectory(
    std::shared_ptr<arrow::fs::FileSystem> fs, std::shared_ptr<ds::ParquetFileFormat> format,
    std::string dir) {
...
void launch(std::shared_ptr<arrow::dataset::ScanTask> task) {
  auto batch_it = task->Execute().ValueOrDie();
...
std::shared_ptr<arrow::Table> GetTableFromScanner(std::shared_ptr<ds::Scanner> scanner) {
  ...
  auto scan_task_iterator = scanner->Scan().ValueOrDie();
  for (auto _scan_task : scan_task_iterator) {
    auto scan_task = _scan_task.ValueOrDie();
    ioService.post(boost::bind(launch, scan_task));
  }
  work.reset();
  threadpool.join_all();
  return nullptr;
